...
export default class VoidClient extends Client {
    ...
    private announceLive(stream: any) {
        ...
        (guild.channels.cache.get("1114960245148614806") as TextChannel).send({
            embeds: [
                new EmbedBuilder()
                    .setAuthor({ name: stream.user_name, iconURL: guild.iconURL() })
                    .setTitle(stream.title)
                    .setURL(`https://twitch.tv/${stream.user_login}`)
                    .setDescription(`${stream.user_name} is now live on [Twitch](https://twitch.tv/${stream.user_login})`)
                    .setColor(0x6441a5)
                    .addFields(
                        { name: 'Game', value: (stream.game_name || "N/A") as string, inline: true },
    ...
    private connectTwitch() {
        const twitch = new Twitch({
            ...
            interval: 1000,
            client: {
                id: 'cmwprnaxw89jbuj60uznv2gulh94hn',
                secret: '2d1wew6buzvqhxgby6q53adzqu73sa',
                token: 'crq8vdzib1vngqgpwxp4uefin5wiob'
            }
        });
